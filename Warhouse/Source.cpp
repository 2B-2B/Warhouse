...
int main() {
	...
	while (!close)
	{
		...
		else if (gui == 1) //Run the AddUser gui
		{
			...
			Warhouse::Display^ display = gcnew Warhouse::Display(&gui, &xPushed); //Creates an intance of the Display gui
			...
			Warhouse::Login^ login = gcnew Warhouse::Login(&gui, &xPushed); //Creates an intance of the Login gui
